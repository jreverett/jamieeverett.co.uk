...
export default function FluidBackground({ reactiveTitleSelector } = {}) {
  ...
  useEffect(() => {
    ...
    // Optional: paint a page title (e.g. the gradient .releases-title) on an overlay
    // canvas that reads the fluid behind it and shifts the glyphs white→cyan (at rest)
    // toward gold over splashes — the same effect as the home-page subtitle. The fluid
    // framebuffer here is 1x (no DPR); the overlay is rendered at DPR for crisp text and
    // samples the 1x fluid readback. Gated by the reactiveTitleSelector prop so other
